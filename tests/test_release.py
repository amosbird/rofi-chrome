...
class ReleaseTest(unittest.TestCase):
    ...
    def test_bookmark_manager_opens_urls_in_a_normal_browser_window(self):
        ...
        self.assertIn("await activateBookmark(event, bookmark)", script)
        self.assertNotIn('checkbox.addEventListener("click", (event) => event.stopPropagation())', script)
        self.assertIn("let openingBookmark = false", script)
        self.assertIn("if (openingBookmark) return", script)
        self.assertIn("Failed to open bookmark in main browser.", script)
